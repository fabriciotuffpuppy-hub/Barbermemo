...
export default function AdminDashboard() {
  ...
  const handleAdminSaveBarber = async (e) => {
    e.preventDefault();
    setAdminError('');
    const isEditing = !!editingBarberId;
    if (
      !adminBarberForm.nome.trim() ||
      !adminBarberForm.email.trim() ||
      (!isEditing && !adminBarberForm.senha.trim()) ||
      !adminBarberForm.barbeariaName.trim()
    ) {
  ...
  const handleAdminEditClick = (barber) => {
    setAdminBarberForm({
      nome: barber.nome,
      email: barber.email,
      senha: '', // A senha não é exibida ou atualizada por aqui por segurança
              ...
              {showAdminForm && (
                <div className="bg-barber-card border border-barber-border rounded-2xl p-5 space-y-4 shadow-xl">
                  ...
                  <form onSubmit={handleAdminSaveBarber} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {!editingBarberId && (
                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-zinc-400 uppercase block font-sans">Senha Provisória de Acesso</label>
                        <div className="relative">
                          <LockKeyhole className="w-4 h-4 text-zinc-500 absolute left-3 top-3" />
                          <input
                            type="text" required placeholder="Ex: Senha123"
                            className="w-full bg-barber-dark border border-barber-border rounded-xl py-2.5 pl-9 pr-3 text-xs text-barber-text-primary"
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-barber-border/40">
                      ...
                      {barbeirosList.length === 0 && !isDataLoading && (
                        <tr>
                          <td colSpan="4" className="py-8 text-center text-zinc-550 font-sans">
